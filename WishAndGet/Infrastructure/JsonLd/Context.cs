...
namespace WishAndGet.Infrastructure.JsonLd
{
	...
	public class Context : JObject
    {
        ...
        internal virtual Context Parse(JToken localContext, List<string> remoteContexts)
		{
        ...
        /// <exception cref="JsonLdError"></exception>
        internal virtual Context Parse(JToken localContext)
		{
			return Parse(localContext, new List<string>());
		}
			...
			, IDictionary<string, bool> defined)
		{
			...
			if (val.ContainsKey("@reverse"))
			{
				...
				string reverse = ExpandIri((string)val["@reverse"], false, true, context, defined
					);
			...
			if (!val["@id"].IsNull() && !val["@id"].SafeCompare(term))
			{
				...
				string res = ExpandIri((string)val["@id"], false, true, context, defined);
        ...
        internal virtual string ExpandIri(string value, bool relative, bool vocab, JObject context, IDictionary<string, bool> defined)
		{
			...
			{
                JToken td = termDefinitions[value];
			...
			, bool reverse)
		{
			...
			if (relativeToVocab && GetInverse().ContainsKey(iri))
			{
				...
				{
					// 2.12.1)
                    string result = CompactIri((string)((IDictionary<string, JToken>)value)["@id"
						], null, true, true);
			...
			foreach (string term_1 in termDefinitions.GetKeys())
			{
				...
				// 5.3)
				string candidate = term_1 + ":" + iri.Substring(((string)termDefinition
					["@id"]).Length);
				// 5.4)
		...
		public object Clone()
		{
            Context rval = new Context(base.DeepClone(),options);
		...
		public virtual JObject Serialize()
		{
			...
			foreach (string term in termDefinitions.GetKeys())
			{
				...
				{
					string cid = CompactIri((string)definition["@id"]);
